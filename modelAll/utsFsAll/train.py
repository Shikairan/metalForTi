#!/usr/bin/env python3
"""
全量 RGAT 训练：604 节点全部参与训练（无 held-out val），交替 MSE(UTS)/MSE(FS)。
按全表 MAE 选优；checkpoint 保存在 modelAll/utsFsAll/runs/。
"""
...
_NUM_RELATIONS = 3
_GAT_HEADS = 4
BEST_CKPT_NAME = "best_rgat_uts_fs_all.pt"
...
def _load_bundle(data_dir: Path):
    for name in ("material_graph.pt", "uts.pt", "fs.pt", "train_mask.pt", "val_mask.pt"):
    ...
    uts = torch.load(data_dir / "uts.pt", map_location="cpu", weights_only=False).reshape(-1).float()
    ...
    # monitor_mask：与 train 同为全 True，用于日志/选优（无 held-out）
    monitor_mask = torch.load(data_dir / "val_mask.pt", map_location="cpu", weights_only=False).reshape(-1).bool()
    return graph, uts, fs, train_mask, monitor_mask
...
def _eval_mae(
    ...
    model.eval()
    with torch.no_grad():
        pred_uts, pred_fs = model(x, edge_index, edge_type)
        mae_uts = F.l1_loss(pred_uts[mask], uts[mask]).item()
        mae_fs = F.l1_loss(pred_fs[mask], fs[mask]).item()
    return mae_uts, mae_fs


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="modelAll/utsFsAll：604 全量训练 UTS+FS")
...
def main() -> None:
    ...
    graph, uts, fs, train_mask, monitor_mask = _load_bundle(args.data_dir)
    ...
    monitor_mask = monitor_mask.to(device)
    ...
    print(
        f"[INFO] nodes={n_nodes}  train={n_train}  monitor=full_set  "
        f"device={device}  targets=UTS+FS  (no held-out val)"
    )
    if n_train != n_nodes:
        raise SystemExit(f"期望 train_mask 覆盖全部 {n_nodes} 节点，实际 train={n_train}")
    ...
    with log_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for epoch in range(1, int(args.epochs) + 1):
            model.train()
            optimizer.zero_grad()
            uts_pred, _ = model(x, edge_index, edge_type)
            loss_uts = F.mse_loss(uts_pred[train_mask], uts[train_mask])
            loss_uts.backward()
            optimizer.step()
            ...
            mae_uts, mae_fs = _eval_mae(
                model, x, edge_index, edge_type, uts, fs, monitor_mask
            )
            full_score = mae_uts + mae_fs
            writer.writerow(
                [
                    epoch,
                    f"{loss_uts.item():.8f}",
                ...
                torch.save(
                    {
                        ...
                        "train_loss": "alternating_mse_uts_then_fs",
                        "in_dim": int(x.shape[1]),
                        "hidden_dim": int(args.hidden_dim),
            ...
            if epoch == 1 or epoch % int(args.log_interval) == 0 or epoch == int(args.epochs):
                print(
                    f"epoch={epoch} "
                    f"train_mse_uts={loss_uts.item():.6f} train_mse_fs={loss_fs.item():.6f} "
                    f"full_mae_uts={mae_uts:.6f} full_mae_fs={mae_fs:.6f} "
                    f"full_score={full_score:.6f} best={best_score:.6f}@{best_epoch}"
                )
