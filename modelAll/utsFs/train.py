#!/usr/bin/env python3
"""
全量 RGAT 训练：604 节点、train:val=1:0.2，交替 MSE(UTS)/MSE(FS)。
模型与 checkpoint 保存在 modelAll/utsFs/runs/。
"""
...
_NUM_RELATIONS = 3
_GAT_HEADS = 4
BEST_CKPT_NAME = "best_rgat_uts_fs.pt"
...
def _load_bundle(data_dir: Path):
    ...
    return graph, uts, fs, train_mask, val_mask
...
def _eval_metrics(
    ...
    with torch.no_grad():
        ...
        train_mae_uts = F.l1_loss(pred_uts[train_mask], uts[train_mask]).item()
        val_mae_uts = F.l1_loss(pred_uts[val_mask], uts[val_mask]).item()
        train_mae_fs = F.l1_loss(pred_fs[train_mask], fs[train_mask]).item()
        val_mae_fs = F.l1_loss(pred_fs[val_mask], fs[val_mask]).item()
    return train_mae_uts, val_mae_uts, train_mae_fs, val_mae_fs


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="modelAll/utsFs 全量 RGAT 训练（UTS+FS）")
...
def main() -> None:
    ...
    graph, uts, fs, train_mask, val_mask = _load_bundle(args.data_dir)
    ...
    print(f"[INFO] nodes={x.shape[0]}  train={n_train}  val={n_val}  device={device}  targets=UTS+FS")
    ...
    with log_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for epoch in range(1, int(args.epochs) + 1):
            ...
            train_mae_uts, val_mae_uts, train_mae_fs, val_mae_fs = _eval_metrics(
                model, x, edge_index, edge_type, uts, fs, train_mask, val_mask
            )
            val_score = val_mae_uts + val_mae_fs
            writer.writerow(
                [
                    epoch,
                    f"{loss_uts.item():.8f}",
            ...
            if epoch == 1 or epoch % int(args.log_interval) == 0 or epoch == int(args.epochs):
                print(
                    ...
                    f"val_mae_uts={val_mae_uts:.6f} val_mae_fs={val_mae_fs:.6f} "
