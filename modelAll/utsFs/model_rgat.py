"""SingleEncoder_DualRGAT — 与 ysFs / r-gatDouble 同结构。

第一头参数名仍为 ys_*（兼容 load_dual_rgat），语义上预测 UTS；第二头为 FS。
"""
...
class SingleEncoder_DualRGAT(nn.Module):
    """独立 UTS/FS encoder → 各 2× RGATConv + LayerNorm + MLP head。"""
    ...
    def __init__(
        ...
        # 参数名保留 ys_*，便于 grd/Pareto 的 load_dual_rgat；训练标签为 UTS
        ...
        self.ys_gat1 = RGATConv(
            hidden_dim, hidden_dim // heads, num_relations, heads=heads, concat=True, dropout=dropout
        )
        self.ys_gat2 = RGATConv(
            hidden_dim, hidden_dim, num_relations, heads=1, concat=False, dropout=dropout
        )
