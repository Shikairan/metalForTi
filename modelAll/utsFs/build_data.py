#!/usr/bin/env python3
"""
从 gnnDir/datacsv/datagnnUts.csv 构建全量图数据，train:val = 1:0.2，
输出到 modelAll/utsFs/data/。标签：UTS + FS。
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="modelAll/utsFs 全量 RGAT 数据构建（UTS+FS，1:0.2 划分）")
    p.add_argument(
        "--csv",
        type=Path,
        default=_GNN_DIR / "datacsv" / "datagnnUts.csv",
        help="特征表（默认 gnnDir/datacsv/datagnnUts.csv，需含 YS/FS/UTS）",
    )
...
def main() -> None:
    ...
    # datagnnUts 含 YS/FS/UTS；构图与划分复用 YS/FS 接口，标签改用 UTS+FS
