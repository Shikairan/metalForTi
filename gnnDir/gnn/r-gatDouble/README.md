...
- **`loop_train_swap_rgat.py`:** one **forward** per epoch, then **L1(YS)** `backward(retain_graph=True)` + **L1(FS)** `backward()`, then **one** `optimizer.step()`. Best checkpoint by **`val_mae_fs + val_mae_ys`**.
...
python3 train_fs_gat.py \
  ...
  --dropout 0.2 \
  --seed 42 \
  --out-dir /home/data/symbolTransformer/symtest/gnnDir/gnn/r-gatDoubel/runs
```
...
[`loop_train_swap_rgat.py`](loop_train_swap_rgat.py) trains **both** heads each round. **Curate/swap** picks val nodes with **largest** `(rel_ys+rel_fs)` and train nodes with **smallest** `(rel_ys+rel_fs)` (same `exp` inverse as FS metrics). Epoch logs: `val_half_rel_pct=[ys_half,fs_half]` (each metric’s **own** half-rank on val), `val_worst_rel_pct=[max_ys_rel%,max_fs_rel%]` on val (**not** necessarily the same node). `val_worst_node_idx` is still the val node maximizing **combined** rel% for mask moves. `val_last_lt10_sorted_numb` counts val nodes with both rel% under 10.
