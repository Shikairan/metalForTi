...
def _eval_metrics(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        # val_half_rel_pct: sort val rel% ascending; take the k-th smallest where
        # k = ceil(n * 50%) = (n+1)//2 for integer n (1-based rank k).
        n_val = int(rel_pct.numel())
        k = max(1, (n_val + 1) // 2)
        sorted_rel, _ = torch.sort(rel_pct)
        val_half_rel_pct = float(sorted_rel[k - 1].item())
        # Ascending sorted index of the last val sample with rel% < 10 (-1 if none).
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Train FS-only R-GCN on material graph data")
    p.add_argument("--data-dir", type=Path, default=base / "gnndataPT" / "r-gnnPT")
    ...
    p.add_argument(
        "--allow-inactive",
        action="store_true",
        help="Allow nodes in neither train nor val (e.g. after loop_train_swap curate phase).",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    _validate_inputs(
        x, fs, train_mask, val_mask, edge_type, allow_inactive=bool(args.allow_inactive)
    )
    ...
    best_epoch = -1
    best_path = args.out_dir / "best_fs_rgcn.pt"
